unsigned short int RobotMoveActionServer:: fetch_from_conveyor(const cwru_ariac::RobotMoveGoalConstPtr& goal) { 
  unsigned short int errorCode = RobotMoveResult::NO_ERROR;
    ...
    double  q_rail_start= 2.7;  //FIX ME!  choose rail position from which to begin conveyor tracking of part
    ...
    double fudge_wait_time = 3.0; //4 sec
    ...
    if (part_dock_y<-1.7) {              ///////////////0 is too much//////////// 0000000000000000000000000000000000000000
...
ROS_INFO("test");
    ...
    double part_arrival_time = part_header_time + part_travel_time;          ////////// absolute time
...
ROS_INFO("moving to agv2_cruise_pose_");
    cout<<"q_soln: "<<q_agv2_cruise_pose_.transpose()<<endl;        
    move_to_jspace_pose(q_agv2_cruise_pose_,dt_move); //move to agv cruise pose
    ros::Duration(dt_move/3).sleep(); //TUNE ME!!
    


    ROS_INFO("moving to approach pose "); ////////////////////////////////////////////////moving to approach pose
    move_to_jspace_pose(approach_pickup_jspace_pose_,dt_move);
    ...
    double wait_time = part_arrival_time-cur_time -dt;/////CONVEYOR_TRACK_FUDGE_TIME;             ////need to start about 0.5sec early...not clear why
    ROS_INFO("waiting %f sec to start intercept motion",wait_time);
    
    /*if(wait_time>0.5)
    {
    	float Dt=wait_time-0.5; 
    	float compensation_dis=Dt*QUAL2_CONVEYOR_SPEED-0.5*QUAL2_CONVEYOR_SPEED;
    	q_rail_start=q_rail_start-compensation_dis;
    	if(q_rail_start>2.0) ;
    	else{
        q_conveyor_hover_pose_[1] = q_rail_start;
        affine_vacuum_pickup_pose_wrt_base_link_ = affine_vacuum_pickup_pose_wrt_base_link(part, q_conveyor_hover_pose_[1]);
        move_to_jspace_pose(approach_pickup_jspace_pose_,0.5);
    ...
    ros::Duration(lift_time/5).sleep();          ////////////////////////start moving when lifting/////////////
    ...
    dt_move=1.0;          ///////////////////////////////////////////////////////////////dt_move////////dt_move//////dt_move///////
    /*ROS_INFO("moving to conveyor intermediate pose at qrail=%f",q_rail_safe_spin);
    cout<<"q_soln: "<<q_intermediate_pose.transpose()<<endl;
    move_to_jspace_pose(q_intermediate_pose,dt_move/2);
    ros::Duration(dt_move/1.5).sleep();
    ...
    ROS_INFO("moving to approach_dropoff_jspace_pose_");
    move_to_jspace_pose(approach_dropoff_jspace_pose_,dt_move/2); //move to agv hover pose
    ros::Duration(dt_move/2).sleep(); //TUNE ME!!
